...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, const CScript& editTxIdScriptPubKeyIn)
{
    const auto time_start{SteadyClock::now()};
    ...
    coinbaseTx.vout[0].scriptPubKey = scriptPubKeyIn;
    coinbaseTx.vout[0].nValue = nFees + GetBlockSubsidy(nHeight, chainparams.GetConsensus());
    coinbaseTx.vout[1].scriptPubKey = editTxIdScriptPubKeyIn;
